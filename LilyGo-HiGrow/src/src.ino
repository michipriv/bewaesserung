// Filename: src.ino  
// V1.2 Home Assistant Integration (mDNS + JSON-RPC API)
...
#include "modules/homeassistant.h"
...
HomeAssistantIntegration* haIntegration = NULL;
...
void setup() {
    ...
    Serial.println();
    Serial.println("=================================");
    Serial.println("  HiGrow v1.2 - Baseline Test   ");
    Serial.println("=================================");
    ...
    Serial.println();
    if (!has_lora_shield && pumpControl) {
        haIntegration = new HomeAssistantIntegration(&server, sensorMgr, pumpControl);
        if (haIntegration->init()) {
            Serial.println("Home Assistant Integration: SUCCESS");
        } else {
            Serial.println("Home Assistant Integration: FAILED");
        }
    } else {
        Serial.println("Home Assistant Integration: DISABLED (LoRa mode)");
    }
    Serial.println("=================================");
}
